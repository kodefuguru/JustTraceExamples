    ...
    public class HardCoverMatrixTransform
    {
        ...
        private static void TransformChanged(object sender, DependencyPropertyChangedEventArgs args)
        {
            ...
            if (matrixTransform != null)
            {
                ...
                {
                    M11 = 1 * Math.Cos(HardCoverMatrixTransform.GetAngle(matrixTransform) * Math.PI / 180),
                    M12 = Math.Sin(HardCoverMatrixTransform.GetAngle(matrixTransform) * Math.PI / 180) * HardCoverMatrixTransform.GetPerspective(matrixTransform),
                    M21 = 0,
                    M22 = 1,
                    OffsetX = 0,
                    OffsetY = 0
